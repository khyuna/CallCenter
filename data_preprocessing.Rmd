...
View(data)
head(data)
data$total = data$count*data$duration #count 칼럼과 duration 칼럼을 곱함
```
...
# add weather column (temp,prec,tempClass,precClass)
write.csv(data,"domino4_1.csv")
```
...
```{r error=FALSE,message=FALSE,warning=FALSE}
data<-read.csv("domino4_2.csv", header=TRUE)
data$
head(data)
```
...
holidayRow$holiday2[570]<-holidayRow$holiday2[569]
...
```{r error=FALSE,message=FALSE,warning=FALSE}
data<-read.csv("domino4_4.csv", header=TRUE)
head(data)
...
```{r error=FALSE,message=FALSE,warning=FALSE}
data<-read.csv("domino4_5.csv", header=TRUE)
head(data)
table(data$baseball)
